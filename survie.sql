...
SELECT visit.fullvisitorid, date_first_visit, date_transactions, date_last_visit , 
       ...
       IFNULL(transactions.transactions,0) AS transactions 
...
SELECT fullvisitorid, date_first_visit, 
       ...
       CASE WHEN device_transaction IS NULL THEN device_last_visit ELSE device_transaction END AS device, transactions 
...
SELECT fullvisitorid, DATE_DIFF(PARSE_DATE('%Y%m%d',date_event), PARSE_DATE('%Y%m%d', date_first_visit),DAY) AS time, 
       device, transactions 
FROM mortality_table ORDER BY transactions DESC, time, fullvisitorid
