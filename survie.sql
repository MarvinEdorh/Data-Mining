...
WHERE _TABLE_SUFFIX BETWEEN '20160801' AND '20160831'GROUP BY fullvisitorid),
...
transactions AS (
SELECT fullvisitorid, MAX(date) AS date_transactions, 1 AS transactions 
FROM `bigquery-public-data.google_analytics_sample.ga_sessions_*` AS ga, UNNEST(ga.hits) AS hits
WHERE _TABLE_SUFFIX BETWEEN '20160801' AND '20160831' AND hits.transaction.transactionId IS NOT NULL
...
SELECT visit.fullvisitorid, date_first_visit, date_transactions, date_last_visit , device_visit.deviceCategory AS device_last_visit, 
device_transactions.deviceCategory AS device_transaction, IFNULL(transactions.transactions,0) AS transactions 
...
CASE WHEN date_transactions IS NULL THEN date_last_visit
ELSE date_transactions  END AS date_event, 
CASE WHEN device_transaction IS NULL THEN device_last_visit ELSE device_transaction END AS device, 
transactions FROM visits_transactions )

SELECT fullvisitorid, DATE_DIFF(PARSE_DATE('%Y%m%d',date_event),PARSE_DATE('%Y%m%d', date_first_visit),DAY) AS time, 
device, transactions FROM mortality_table
ORDER BY transactions DESC, time, fullvisitorid